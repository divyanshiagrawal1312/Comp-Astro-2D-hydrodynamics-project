...
class RiemannProblem:
    ...
    def u_hugoniot(self, p, side):
        ...
        # u = np.where(
        #     p < state.p,
        #     state.u + s * (2.0 * c / (self.gamma - 1.0)) * (1.0 - (p / state.p) ** ((self.gamma - 1.0) / (2.0 * self.gamma))),
        #     state.u + s * (2.0 * c / np.sqrt(2.0 * self.gamma * (self.gamma - 1.0))) * (1.0 - p / state.p) / np.sqrt(1.0 + ((self.gamma + 1.0) / \
        #     (self.gamma - 1.0)) * p / state.p)
        # )
